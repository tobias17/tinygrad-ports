...
class FrozenClosedClipEmbedder(Embedder):
  ...
  def __call__(self, texts:Union[str,List[str],Tensor]) -> Dict[str,Tensor]:
    ...
    return { "prompt_embed": self.transformer.text_model(tokens.reshape(len(texts),-1)) }
...
class FrozenOpenClipEmbedder(Embedder):
  ...
  def embed_tokens(self, tokens:Tensor) -> Dict[str,Tensor]:
    ...
    ret_data = { "prompt_embed": penultimate }

    if self.return_pooled:
      x = self.model.ln_final(x)
      index = tokens.argmax(axis=-1).reshape(-1,1,1).expand(x.shape[0],1,x.shape[-1])
      ret_data["add_text_embed"] = x.gather(1, index).squeeze(1) @ self.model.text_projection
