...
class UNetModel:
  ...
  def old_call(self, x:Tensor, tms:Tensor, ctx:Tensor, y:Optional[Tensor]=None, idx=-1) -> Tensor:
    t_emb = timestep_embedding(tms, self.model_ch)
    emb   = t_emb.sequential(self.time_embed)

    # import numpy as np
    # compare_text_emb = Tensor(np.load("first_call_text_embeds.npy"), dtype=dtypes.float16).realize()
    # compare_time_emb = Tensor(np.load("first_call_text_embeds.npy"), dtype=dtypes.float16).realize()

    if y is not None:
      assert y.shape[0] == x.shape[0]
      emb = emb + y.sequential(self.label_emb[0])

    # compare_y   = Tensor(np.load("first_call_y.npy"),   dtype=dtypes.float16).realize()
    # compare_ctx = Tensor(np.load("first_call_ctx.npy"), dtype=dtypes.float16).realize()
    # compare_emb = Tensor(np.load("first_call_emb.npy"), dtype=dtypes.float16).realize()
    # print(f"y   ours={y.shape} comp={compare_y.shape}")
    # print(f"ctx ours={ctx.shape} comp={compare_ctx.shape}")
    # print(f"emb ours={emb.shape} comp={compare_emb.shape}")
    # print(f"y:   {(y   - compare_y  ).abs().mean().numpy()}")
    # print(f"ctx: {(ctx - compare_ctx).abs().mean().numpy()}")
    # print(f"emb: {(emb - compare_emb).abs().mean().numpy()}")
    # assert False

    # ctx = Tensor(np.load(f"stages/{idx:02d}/ctx.npy"), dtype=dtypes.float16).realize()
    # emb = Tensor(np.load(f"stages/{idx:02d}/emb.npy"), dtype=dtypes.float16).realize()
